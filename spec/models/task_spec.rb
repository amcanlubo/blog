...
RSpec.describe Task, type: :model do
  ...
  let(:task) { Task.create!(category_id: category.id, name:'A valid task name', body:"A valid task body",task_date: Date.current)}
    ...
    context 'Task Validations' do
      ...
      it '5. Is not valid when name has less than 6 characters' do
        # task.name = 'a' * 5
        task = category.tasks.new(category_id: Category.last.id, name:'a' * 5, body:'A valid task body',task_date: Date.current)
